...
def patch_values_yaml(
    ...
    def split_imagefrom(value: str) -> Optional[Tuple[str, str]]:
        """
        Parse imagefrom by splitting on the last underscore.

        Examples:
          jdksemeru21_21.0.29 -> ("jdksemeru21", "21.0.29")
          ttujdk17_20.0.34    -> ("ttujdk17", "20.0.34")
        """
        ...
        family, version = normalized.rsplit("_", 1)
    ...
    def apply_imagefrom_patch(
        ...
        if not isinstance(cur_value, str):
            notes.append(
                f"{path_prefix} ignored: imagefrom value is not a string"
            )
            return

        cur_norm = cur_value.strip()
        parsed = split_imagefrom(cur_norm)

        if not parsed:
            notes.append(
                f"{path_prefix} ignored: unsupported imagefrom format ({cur_norm})"
            )
            return

        family, _current_version = parsed

        tgt = normalize_str(imagefrom_policy.get(family, ""))

        # Family not managed by config.yaml => leave untouched
        if not tgt:
            return

        parsed_target = split_imagefrom(tgt)

        if not parsed_target:
            notes.append(
                f"{path_prefix} ignored: invalid target in config.yaml "
                f"for family {family}: {tgt}"
            )
            return

        target_family, _target_version = parsed_target

        # Safety guard: a ttujdk17 value can never receive a jdksemeru17 target.
        if target_family != family:
            notes.append(
                f"{path_prefix} ignored: target {tgt} does not belong "
                f"to family {family}"
            )
            return
        ...
        set_value_fn(tgt)
        changed = True
        notes.append(f"{path_prefix}: {cur_norm} -> {tgt}")

    def walk(obj: Any, path: str = ""):
        if isinstance(obj, dict):
            # Pattern 0:
            # imagefrom: "jdksemeru21_21.0.29"
            # imagefrom: "ttujdk17_20.0.34"
            #
            # Process every occurrence recursively and independently.
            for k in list(obj.keys()):
                if str(k).strip().lower() == "imagefrom":
                    apply_imagefrom_patch(
                        cur_value=obj[k],
                        set_value_fn=lambda v, d=obj, key=k: d.__setitem__(key, v),
                        path_prefix=f"{path}{k}",
                    )
    ...
    if not validate_yaml(out):
        raise ValueError("invalid YAML generated after values patch")
...
def main():
    ...
    policy_deps = policy_cfg.get("helm_dependencies", {}) or {}
    images_policy = policy_cfg.get("images", {}) or {}
    imagefrom_policy = policy_cfg.get("imagefrom", {}) or {}
    ...
    for p in projects:
        ...
        for vp in values_candidates:
            ...
            if txt is None:
                continue
            try:
                new_txt, changed, notes = patch_values_yaml(
                    values_text=txt,
                    images_policy=images_policy,
                    imagefrom_policy=imagefrom_policy,
